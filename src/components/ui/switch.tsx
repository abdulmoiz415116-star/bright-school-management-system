...
export function Switch({ checked = false, onCheckedChange, className = "" }: SwitchProps) {
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${checked ? 'bg-blue-600' : 'bg-slate-200'} ${className}`}
    >
      <span
