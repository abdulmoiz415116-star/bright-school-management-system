...
export function AdmissionClient({ initialStudents = [] }: { initialStudents?: Student[] }) {
  ...
  return (
    <div className="w-full space-y-8">
      ...
      <form onSubmit={handleSubmit} className="print-container bg-white dark:bg-slate-950/80 rounded-3xl border border-pink-200/80 dark:border-pink-900/40 shadow-xl overflow-hidden p-6 sm:p-10 space-y-8">
        
        {/* OFFICIAL SCHOOL HEADER (Visible on screen and Print) */}
        <div className="border-b-2 border-slate-900 dark:border-pink-800/60 pb-6 flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
          <div className="flex flex-col md:flex-row items-center gap-4">
            <img 
              src="/school_logo.png" 
              alt="Bright School & Montessori System Official Crest" 
            ...
            <div>
              ...
              <p className="text-rose-600 dark:text-rose-400 font-bold text-sm sm:text-base mt-0.5">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-1">
                Regd No: BS-EDU-9042 | Main Campus, Education Zone | Ph: 0300-1234567
              </p>
            </div>
          </div>
          <div className="flex flex-col items-end gap-2 bg-pink-50 dark:bg-pink-950/60 p-4 rounded-2xl border border-pink-200/60 dark:border-pink-800/40 shrink-0 min-w-[200px] text-center md:text-right">
            ...
            <span className="text-base font-black text-slate-900 dark:text-white font-mono bg-white dark:bg-slate-900 px-3 py-1 rounded-lg border border-pink-200/80 dark:border-pink-800/50 shadow-sm">
              Form No: {formNo}
            </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
          ...
          <div className="md:col-span-3 space-y-6">
            <div className="bg-slate-50 dark:bg-slate-900/50 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
              <h3 className="text-lg font-black text-slate-900 dark:text-pink-100 flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-2">
                <Sparkles className="w-5 h-5 text-amber-500" />
                {isUrdu ? '1۔ مطلوبہ کلاس کا انتخاب (Admission Sought For)' : '1. Admission Sought For'}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <Label htmlFor="class_applying" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
                  ...
                  <select
                    ...
                    className="w-full mt-1.5 h-11 px-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 font-bold text-sm focus:ring-2 focus:ring-rose-500"
                  >
                ...
                <div>
                  <Label htmlFor="shift" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
                    {isUrdu ? 'شفٹ (Shift)' : 'Shift'}
                  </Label>
                  <select id="shift" className="w-full mt-1.5 h-11 px-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 font-bold text-sm">
                    <option value="Morning">Morning Shift (صبح)</option>
                    <option value="Afternoon">Afternoon Shift (شام)</option>
                  </select>
                </div>
                <div>
                  <Label htmlFor="previous_school" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
                  ...
                  <Input
                    ...
                    onChange={handleInputChange}
                    className="mt-1.5 h-11 rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col items-center justify-center p-3 rounded-2xl border-2 border-dashed border-rose-300 dark:border-rose-800 bg-rose-50/40 dark:bg-rose-950/20 text-center relative overflow-hidden min-h-[210px] group">
            {photoPreview ? (
              <div className="relative w-full h-full flex flex-col items-center">
                <img 
                  src={photoPreview} 
                  alt="Candidate Photo" 
                  className="w-32 h-40 object-cover rounded-xl border-2 border-white shadow-md mb-2"
                />
                <button
                  type="button"
                  onClick={handleRemovePhoto}
                  className="no-print bg-rose-600 hover:bg-rose-700 text-white p-1.5 rounded-full text-xs font-bold shadow-md transition flex items-center gap-1 px-3"
                >
                  <Trash2 className="w-3.5 h-3.5" /> {isUrdu ? 'تصویر ہٹائیں' : 'Remove Photo'}
                </button>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center py-4 px-2">
                <User className="w-12 h-12 text-rose-400 mb-2" />
                <span className="text-xs font-bold text-slate-700 dark:text-slate-200 mb-1">
                  {isUrdu ? 'طالب علم کی تصویر اپلوڈ کریں' : 'Upload Candidate Photo'}
                </span>
                <span className="text-[10px] text-slate-500 mb-3">(Passport size, Blue background)</span>
                
                <label className="no-print bg-gradient-to-r from-rose-600 to-indigo-600 hover:from-rose-700 hover:to-indigo-700 text-white px-4 py-2 rounded-xl text-xs font-bold cursor-pointer shadow-md transition flex items-center gap-2">
                  <Upload className="w-3.5 h-3.5" />
                  {isUrdu ? 'فائل منتخب کریں' : 'Select Photo File'}
                  <input 
                    type="file" 
                    accept="image/*" 
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
            <div>
              <Label htmlFor="student_name" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
              ...
              <Input
                ...
                onChange={handleInputChange}
                className="mt-1.5 h-11 rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 font-semibold"
              />
            </div>
            <div>
              <Label htmlFor="gender" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
              ...
              <select
                ...
                onChange={handleInputChange}
                className="w-full mt-1.5 h-11 px-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 font-semibold text-sm"
              >
            ...
            <div>
              <Label htmlFor="dob" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
            ...
            <div>
              <Label htmlFor="cnic" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
              ...
              <Input
                ...
                value={formData.cnic}
                onChange={handleInputChange}
                className="mt-1.5 h-11 rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 font-mono"
              />
            </div>
            <div>
              <Label htmlFor="blood_group" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
            ...
            <div>
              <Label htmlFor="religion" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
            <div>
              <Label htmlFor="father_name" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
              ...
              <Input
                ...
                name="father_name"
                required
                placeholder={isUrdu ? 'والد کا مکمل نام' : "e.g. Tariq Mahmood"}
            ...
            <div>
              <Label htmlFor="father_cnic" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
            ...
            <div>
              <Label htmlFor="father_occupation" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
                {isUrdu ? 'والد کا پیشہ / کاروبار' : "Father's Occupation"}
              </Label>
              <Input
                id="father_occupation"
                name="father_occupation"
                placeholder={isUrdu ? 'مثال: بزنس، ملازمت' : 'e.g. Engineer / Business'}
            ...
            <div>
              <Label htmlFor="mobile_number" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
            ...
            <div>
              <Label htmlFor="whatsapp_number" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
            ...
            <div>
              <Label htmlFor="mother_name" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="md:col-span-2">
              <Label htmlFor="address" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
            ...
            <div>
              <Label htmlFor="emergency_contact" className="font-bold text-xs uppercase text-slate-700 dark:text-slate-300">
        ...
        <div className="space-y-4 bg-pink-50/50 dark:bg-pink-950/20 p-6 rounded-2xl border border-pink-200/60 dark:border-pink-800/40">
          <h3 className="text-lg font-black text-slate-900 dark:text-pink-100 flex items-center gap-2 border-b border-pink-200 dark:border-pink-900/60 pb-2">
            <FileCheck2 className="w-5 h-5 text-amber-600" />
            {isUrdu ? '5۔ ضروری دستاویزات اور اقرار نامہ (Documents & Undertaking)' : '5. Required Documents & Undertaking'}
          </h3>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs font-bold text-slate-700 dark:text-slate-300 py-2">
            <div className="flex items-center gap-2 bg-white dark:bg-slate-900 p-3 rounded-xl border border-pink-200/50">
              <input type="checkbox" defaultChecked className="rounded text-rose-600 w-4 h-4" />
              <span>4 Passport Photos (4 تصاویر)</span>
            </div>
            <div className="flex items-center gap-2 bg-white dark:bg-slate-900 p-3 rounded-xl border border-pink-200/50">
              <input type="checkbox" defaultChecked className="rounded text-rose-600 w-4 h-4" />
              <span>B-Form Copy (بی فارم کاپی)</span>
            </div>
            <div className="flex items-center gap-2 bg-white dark:bg-slate-900 p-3 rounded-xl border border-pink-200/50">
              <input type="checkbox" defaultChecked className="rounded text-rose-600 w-4 h-4" />
              <span>Father CNIC Copy (شناختی کارڈ)</span>
            </div>
            <div className="flex items-center gap-2 bg-white dark:bg-slate-900 p-3 rounded-xl border border-pink-200/50">
              <input type="checkbox" defaultChecked className="rounded text-rose-600 w-4 h-4" />
              <span>Result Card / SLC (سابقہ سند)</span>
            </div>
          </div>
          ...
          <div className="pt-2">
            <label className="flex items-start gap-3 cursor-pointer select-none">
              <input
                ...
                onChange={handleInputChange}
                className="mt-1 w-5 h-5 rounded border-pink-400 text-rose-600 focus:ring-rose-500 shrink-0"
              />
              <span className="text-xs sm:text-sm font-semibold text-slate-800 dark:text-slate-200 leading-relaxed">
                {isUrdu 
                  ? 'میں اقرار کرتا/کرتی ہوں کہ تمام درج شدہ معلومات بالکل درست ہیں۔ میں برائٹ سکول اینڈ مونٹیسوری سسٹم کے تمام قواعد و ضوابط، فیس شیڈول اور نظم و ضبط کی پابندی کا پابند ہوں گا۔'
                  : 'I hereby declare that the information provided is correct to the best of my knowledge. I agree to abide by the rules, discipline, and fee policy of Bright School & Montessori System.'
                }
              </span>
            </label>
          </div>
        ...
        <div className="space-y-6 pt-4 border-t-2 border-slate-200 dark:border-slate-800">
          <div className="flex items-center justify-between no-print">
            <h3 className="text-base font-black uppercase tracking-wider text-slate-800 dark:text-slate-200 flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/60 px-3 py-1 rounded-full border border-rose-200 dark:border-rose-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3">
              ...
              <div className="space-y-2">
                <Label className="text-[11px] font-bold text-slate-500 no-print">
                ...
                <Input
                  ...
                  className="h-10 rounded-xl font-semibold no-print"
                />
                <div className="h-14 bg-white dark:bg-slate-950 rounded-xl border border-slate-300 dark:border-slate-700 flex items-center justify-center px-3 text-center">
                  <span className="signature-font text-2xl text-slate-800 dark:text-slate-100 font-bold truncate">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
                ...
                <Badge variant="secondary" className="text-[10px] bg-indigo-500/10 text-indigo-600">Officer Verified</Badge>
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                <div className="h-14 bg-white dark:bg-slate-950 rounded-xl border border-slate-300 dark:border-slate-700 flex items-center justify-center px-3 text-center">
                  <span className="signature-font text-2xl text-indigo-700 dark:text-indigo-400 font-bold truncate">
            ...
            <div className="bg-rose-50/50 dark:bg-rose-950/20 p-4 rounded-2xl border border-rose-200/80 dark:border-rose-900/40 space-y-3">
              <div className="flex items-center justify-between border-b border-rose-200 dark:border-rose-900 pb-2">
                ...
                <div className="flex items-center gap-1 no-print">
                  <button
                    ...
                    className={`px-2 py-0.5 text-[10px] font-bold rounded-lg transition ${principalStatus === 'Approved' ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-700'}`}
                  >
                    Approve ✅
                  </button>
                  <button
                    ...
                    className={`px-2 py-0.5 text-[10px] font-bold rounded-lg transition ${principalStatus === 'Pending' ? 'bg-amber-500 text-white' : 'bg-slate-200 text-slate-700'}`}
                  >
                    Pending ⏳
                  </button>
                </div>
              ...
              <div className="flex items-center justify-between gap-2 pt-1">
                <div className="space-y-1">
                  <p className="text-xs font-bold text-slate-800 dark:text-slate-100">{principalName}</p>
                  <p className="text-[10px] text-slate-500 font-medium">Principal, Bright School</p>
                  <Badge className={principalStatus === 'Approved' ? 'bg-emerald-600 text-white text-[10px]' : 'bg-amber-500 text-white text-[10px]'}>
                    {principalStatus === 'Approved' ? 'APPROVED & SIGNED' : 'UNDER REVIEW'}
                  </Badge>
                </div>

                {/* OFFICIAL EMBOSSED SEAL STAMP */}
                <div className="w-20 h-20 rounded-full border-4 border-dashed border-rose-600/60 dark:border-rose-400/60 flex flex-col items-center justify-center text-center p-1 transform rotate-[-6deg] bg-rose-500/10 shrink-0">
                  <span className="text-[7px] font-black uppercase tracking-tighter text-rose-700 dark:text-rose-300">BRIGHT SCHOOL</span>
                  <Stamp className="w-5 h-5 text-rose-600 my-0.5" />
                  <span className="text-[6px] font-black uppercase text-rose-800 dark:text-rose-200">SEAL APPROVED</span>
                </div>
              </div>
            </div>
