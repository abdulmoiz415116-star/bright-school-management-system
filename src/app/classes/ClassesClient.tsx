...
export function ClassesClient({ 
  ...
  const mergedStudents = useMemo(() => {
    const baseList = [...dummyStudents];
    initialStudents.forEach((realStudent) => {
      const exists = baseList.some(d => d.admission_number === realStudent.admission_number || d.id === realStudent.id);
      if (!exists) {
        baseList.push(realStudent);
      }
    });
